...
> **三端同步（GitHub 唯一基准）**：`本地 --push--> GitHub --pull--> 服务器`。本地用 `scripts/publish.sh` 推送；服务器用 `deploy/sync.sh` 拉齐。服务器私有配置（`deploy/Caddyfile.prod`、`.env`）被 `.gitignore` 隔离。详见 [deploy/DEPLOY.md](deploy/DEPLOY.md)。
